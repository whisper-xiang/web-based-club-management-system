				...
				<td>
					<table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
						<tr align="left" bgcolor="#F2FDFF">
							<td height="30" colspan="17" background="images/table_header.gif">活动报名列表</td>
						</tr>
					</table>
				</td>
			</tr>
						...
						<tr align="left" >
							<td colspan="17">
								<form action="huodongbaomingList.do" name="myform" method="post">
									...
									<input type="submit" value="查询" class="btn btn-info btn-small" /> <input type="button" value="导出excel" onClick="javascript:location.href='huodongbaoming_listxls.jsp';" class="btn btn-info btn-small" />
								</form>								</td>
						</tr></table>
					  </td>
			    </tr>
				...
				%>
                            <tr align="center">
                             ...
                             <td>${u.yonghuming}</td>    <td>${u.xingming}</td>    <td>${u.shetuanmingcheng}</td>    <td>${u.shetuanleixing}</td>    <td>${u.huodongzhuti}</td>    <td>${u.huodongdidian}</td>    <td>${u.huodongshijian}</td>    <td>${u.baomingren}</td>    <td>${u.baomingliyou}</td>    <td align='center'>${u.issh} <input type="button" value="审核" onclick="javascript:location.href='sh.jsp?id=${u.id}&yuan=${u.issh}&tablename=huodongbaoming'"/></td>
                              ...
                              <td height="50" align="center"><a class="btn btn-info btn-small" href="doUpdateHuodongbaoming.do?id=${u.id }">编辑</a>  <a class="btn btn-info btn-small" href="deleteHuodongbaoming.do?id=${u.id }"
										onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a class="btn btn-info btn-small" href="huodongbaomingDetail.do?id=${u.id}">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
