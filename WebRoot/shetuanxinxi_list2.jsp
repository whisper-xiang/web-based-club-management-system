...
<html xmlns="http://www.w3.org/1999/xhtml">
			  ...
			  <table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
					...
					<tr align="right" bgcolor="#ebf0f7">
					  ...
					  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
						<tr align="left" >
							<td colspan="17">
								<form action="shetuanxinxiList2.do" name="myform" method="post">
									查询   用户名：<input name="yonghuming" type="text" id="yonghuming" class="form-control2" />  姓名：<input name="xingming" type="text" id="xingming" class="form-control2" />  社团名称：<input name="shetuanmingcheng" type="text" id="shetuanmingcheng" class="form-control2" /> 社团类型：<select name='shetuanleixing' id='shetuanleixing' class="form-control2"><option value="">所有</option><%=connDbBean.hsggetoption("shetuanfenlei","shetuanleixing")%></select>  地址：<input name="dizhi" type="text" id="dizhi" class="form-control2" />
    ...
    <td align='center' bgcolor='#ebf0f7'>概要</td>
    ...
    <td align='center'>${u.issh} </td>
                              ...
                              <td height="45" align="center"><a class="btn btn-info btn-small" href='shetuanhuodong_add.jsp?id=${u.id}'>社团活动</a> <a class="btn btn-info btn-small" href="deleteShetuanxinxi.do?id=${u.id }" onClick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a class="btn btn-info btn-small" href="doUpdateShetuanxinxi.do?id=${u.id }">编辑</a>  <a class="btn btn-info btn-small" href="stxxDetail.do?id=${u.id }" target="_blank">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
		 ...
		 <c:if test="${page.page>1}">
              <a href="shetuanxinxiList2.do?page=1" >首页</a>
             <a href="shetuanxinxiList2.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="shetuanxinxiList2.do?page=${page.page+1 }">下一页</a>
			<a href="shetuanxinxiList2.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
