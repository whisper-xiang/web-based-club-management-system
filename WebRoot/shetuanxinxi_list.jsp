			  ...
			  <table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
						...
						<tr align="left" >
							<td colspan="17">
								<form action="shetuanxinxiList.do" name="myform" method="post">
									...
									<input type="submit" value="查询" class="btn btn-info btn-small" /> <input type="button" value="导出excel" onClick="javascript:location.href='shetuanxinxi_listxls.jsp';" class="btn btn-info btn-small" />
								</form>								</td>
						</tr></table>
					  </td>
			    </tr>
				...
				%>
                            <tr align="center">
                             ...
                             <td>${u.yonghuming}</td>    <td>${u.xingming}</td>    <td>${u.shetuanmingcheng}</td>    <td>${u.shetuanleixing}</td>    <td width='90' align='center' bgcolor='#ebf0f7'><a href='${u.tupian}' target='_blank'><img src='${u.tupian}' width=88 height=99 border=0 /></a></td>    <td>${u.dizhi}</td>    <td>${u.gaiyao}</td>        <td align='center'>${u.issh} <input type="button" value="审核" onclick="javascript:location.href='sh.jsp?id=${u.id}&yuan=${u.issh}&tablename=shetuanxinxi'"/></td>
                              <td height="50" align="center" > ${u.addtime } </td>
                              <td height="50" align="center"><a class="btn btn-info btn-small" href="doUpdateShetuanxinxi.do?id=${u.id }">编辑</a>  <a class="btn btn-info btn-small" href="deleteShetuanxinxi.do?id=${u.id }"
										onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a class="btn btn-info btn-small" href="stxxDetail.do?id=${u.id}" target="_blank">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
